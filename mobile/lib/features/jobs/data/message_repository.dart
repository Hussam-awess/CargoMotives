...
class ChatMessage {
  const ChatMessage({required this.id, required this.body, required this.isMine, required this.readAt, required this.createdAt});
...
class MessageRepository {
  ...
  Future<ChatMessage> send(int jobId, String body) async {
    final response = await _client.post('/jobs/$jobId/messages', data: {'body': body});
