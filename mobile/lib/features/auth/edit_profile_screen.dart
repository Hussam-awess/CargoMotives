...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  late final _secondaryController = TextEditingController(
    text: _currentSecondaryValue == '—' ? '' : _currentSecondaryValue,
  );
  ...
  String get _credentialLabel =>
      widget.credential == ProfileCredential.email ? 'Email' : 'Phone number';
  String get _currentCredentialValue =>
      (widget.credential == ProfileCredential.email
          ? _profile.email
          : _profile.phoneNumber) ??
      '—';

  String get _secondaryLabel =>
      widget.credential == ProfileCredential.email ? 'Phone number' : 'Email';
  String get _currentSecondaryValue =>
      (widget.credential == ProfileCredential.email
          ? _profile.phoneNumber
          : _profile.email) ??
      '—';
  ...
  Future<void> _saveSecondary() async {
    ...
    if (value.isEmpty) {
      setState(
        () => _secondaryError = 'Enter your ${_secondaryLabel.toLowerCase()}.',
      );
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('$_secondaryLabel updated.')));
    } on ApiException catch (e) {
      final field = widget.credential == ProfileCredential.email
          ? 'phone_number'
          : 'email';
  ...
  Future<void> _saveBusinessIdentity() async {
    ...
    try {
      ...
      final updated = await widget.authRepository.updateBusinessIdentity(
      ...
      setState(
        () => _businessError =
            e.firstErrorFor('company_name') ??
            e.firstErrorFor('logo') ??
            e.message,
      );
    } finally {
  ...
  Future<void> _sendCode() async {
    ...
    if (newValue.isEmpty) {
      setState(
        () =>
            _credentialError = 'Enter a new ${_credentialLabel.toLowerCase()}.',
      );
      return;
    }
    ...
    try {
      final updated = widget.credential == ProfileCredential.email
          ? await widget.authRepository.confirmEmailChange(
              newEmail: _pendingNewValue!,
              code: code,
            )
          : await widget.authRepository.confirmPhoneChange(
              newPhone: _pendingNewValue!,
              code: code,
            );
      if (!mounted) return;
      setState(() {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('$_credentialLabel updated.')));
    } on ApiException catch (e) {
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              keyboardType: widget.credential == ProfileCredential.email
                  ? TextInputType.phone
            ...
            if (_isCustomer) ...[
              ...
              Text(
                'Shown to transporters bidding on your jobs.',
                style: TextStyle(
                  fontSize: 12.5,
      ...
      children: [
        CircleAvatar(
          ...
          child: _profile.avatarUrl == null
              ? Icon(Icons.person_outline, size: 40, color: AppColors.ctaBlue)
              : null,
        ),
    ...
    switch (_credentialStep) {
      case _CredentialStep.idle:
        return Row(
          children: [
            Expanded(
              child: Text(
                _currentCredentialValue,
                style: const TextStyle(fontSize: 15),
              ),
            ),
            TextButton(
              onPressed: () => setState(
                () => _credentialStep = _CredentialStep.enteringValue,
              ),
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                labelText: 'New $_credentialLabel'.toLowerCase(),
      ...
      case _CredentialStep.enteringCode:
        return Column(
          ...
          children: [
            Text(
              'We sent a code to $_pendingNewValue.',
