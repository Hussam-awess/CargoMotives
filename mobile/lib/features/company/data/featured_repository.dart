...
class PreferredRoute {
  ...
  factory PreferredRoute.fromJson(Map<String, dynamic> json) {
    return PreferredRoute(origin: json['origin'] as String, destination: json['destination'] as String);
  }

  final String origin;
  final String destination;

  Map<String, dynamic> toJson() => {'origin': origin, 'destination': destination};
}
...
class CompanyFeaturedStatus {
  ...
  factory CompanyFeaturedStatus.fromJson(Map<String, dynamic> json) {
    return CompanyFeaturedStatus(
      ...
      preferredRoutes: (json['preferred_routes'] as List).map((e) => PreferredRoute.fromJson(e as Map<String, dynamic>)).toList(),
    );
  }
...
class FeaturedPayment {
  ...
  factory FeaturedPayment.fromJson(Map<String, dynamic> json) => FeaturedPayment(status: json['status'] as String);
...
class CompanyFeaturedRepository {
  CompanyFeaturedRepository({ApiClient? client}) : _client = client ?? ApiClient();
  ...
  Future<FeaturedPayment> purchase({required String provider, required String phoneNumber}) async {
    final body = await _client.post('/company/featured/purchase', data: {'mobile_money_provider': provider, 'phone_number': phoneNumber});

    return FeaturedPayment.fromJson(body['data'] as Map<String, dynamic>);
  }

  Future<List<PreferredRoute>> updatePreferredRoutes(List<PreferredRoute> routes) async {
    final body = await _client.post('/company/featured/preferred-routes', data: {'routes': routes.map((r) => r.toJson()).toList()});

    return (body['preferred_routes'] as List).map((e) => PreferredRoute.fromJson(e as Map<String, dynamic>)).toList();
  }
}
