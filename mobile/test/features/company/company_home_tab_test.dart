...
void main() {
  testWidgets('shows the company name and a zero-activity dashboard when nothing is happening', (tester) async {
  ...
  testWidgets('derives real stats and shows the active job', (tester) async {
    await tester.pumpWidget(
      _appUnder(
        CompanyHomeTab(
          ...
          truckRepository: FakeTruckRepository(onList: () async => [_truck(1), _truck(2)]),
          driverRepository: FakeDriverRepository(onList: () async => [_driver(1, isActive: true), _driver(2, isActive: false)]),
        ),
      ),
    );
    await tester.pumpAndSettle();
    ...
    expect(find.text('Tanga → Dodoma'), findsWidgets); // active job + matching load
...
Driver _driver(int id, {required bool isActive}) {
  return Driver(id: id, fullName: 'Driver $id', phoneNumber: '+2557000000$id', licenseNumber: null, photoUrl: null, isActive: isActive);
}
