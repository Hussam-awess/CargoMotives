...
void main() {
  testWidgets('shows no badge when there are no unread notifications', (tester) async {
    await tester.pumpWidget(
      _appUnder(Scaffold(appBar: AppBar(actions: [NotificationBellButton(repository: FakeNotificationRepository(onUnreadCount: () async => 0))]))),
    );
  ...
  testWidgets('shows the unread count as a badge', (tester) async {
    await tester.pumpWidget(
      _appUnder(Scaffold(appBar: AppBar(actions: [NotificationBellButton(repository: FakeNotificationRepository(onUnreadCount: () async => 3))]))),
    );
    await tester.pumpAndSettle();
