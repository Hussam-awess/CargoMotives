...
class FakeBidRepository extends BidRepository {
  FakeBidRepository({
    ...
    this.onWithdraw,
    this.onAccept,
    this.onCompanyQuotaRemaining,
  });

  final Future<List<Bid>> Function(int jobId)? onForJob;
  final Future<Bid> Function({
    required int jobId,
  ...
  @override
  Future<List<Bid>> forJob(int jobId) =>
      onForJob?.call(jobId) ?? Future.value(const []);
  ...
  @override
  Future<Bid> withdraw(int bidId) =>
      onWithdraw?.call(bidId) ??
      Future.value(_defaultBid(jobId: 1, price: 100000));
  ...
  @override
  Future<int> companyQuotaRemaining() =>
      onCompanyQuotaRemaining?.call() ?? Future.value(5);
}
