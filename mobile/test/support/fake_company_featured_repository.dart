...
class FakeCompanyFeaturedRepository extends CompanyFeaturedRepository {
  FakeCompanyFeaturedRepository({
    this.onStatus,
    this.onPurchase,
    this.onUpdatePreferredRoutes,
  });

  final Future<CompanyFeaturedStatus> Function()? onStatus;
  final Future<FeaturedPayment> Function({
    required String provider,
    required String phoneNumber,
  })?
  onPurchase;
  final Future<List<PreferredRoute>> Function(
    List<PreferredRoute> routes, {
    String? homeRegion,
  })?
  onUpdatePreferredRoutes;
  ...
  Future<FeaturedPayment> purchase({
    ...
    return onPurchase?.call(provider: provider, phoneNumber: phoneNumber) ??
        Future.value(const FeaturedPayment(status: 'pending_confirmation'));
  }
  ...
  Future<List<PreferredRoute>> updatePreferredRoutes(
    ...
    String? homeRegion,
      ...
      onUpdatePreferredRoutes?.call(routes, homeRegion: homeRegion) ??
      Future.value(routes);
}
